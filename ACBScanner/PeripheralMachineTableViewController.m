...
@interface PeripheralMachineTableViewController ()<CBPeripheralManagerDelegate,AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.brightnessVlaueArr = [NSMutableArray arrayWithCapacity:10];
    self.config = [ACBScannerCongfig unArchiver];
    self.operator = self.config.worker.name;
    self.jobNumber = self.config.worker.number;
